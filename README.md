...
1.作用：当页面中要展示的数据较多时，可以使用分页展示，查看更方便。

2.使用注意点：twbPagination插件是jQuery插件，必须先引入jQuery；样式采用bootstrap样式，必须再引入boot的css文件。

3.使用步骤：给指定元素调用twbPagination()，传入分页功能的配置信息，对象，配置的具体名称见文档。

4.总页数：totalPages：28；当前一共显示几个页码：visiblePages：10；first：‘首页’；last：‘尾页’；prev：‘上一页’；next：‘下一页’；onPageClick:funciton() {};
...
- 富文本编辑器为什么要使用富文本编辑器工具:
  
  ​    \- 表单中输入大量的内容时使用textarea
  
  ​    \- 如果希望输入的大量内容时具有格式的，可以使用一些工具，称为富文本编辑器
  ...
  ​     \- 超文本：通过普通文本描述富文本的一种方式。（最终的结果是富文本）
  
  ​    \- 富文本编辑器，本质上就是通过一个工具，让页面中的某个textarea具备更多的功能，最终输出的是富文本形式的内容
  
  ​     \- 实现方式：通过设置好的功能，让某些普通文本被某些标签包裹
  
  ​      \- 最终保存的是具有标签的文本内容，到页面中展示时，具有标签的部分就有格式了
  
  
  
     使用方式：
  
  ​    \- 文档是英文，如果看不懂，可以百度找其他人的使用感想(小教程)
  
  ​    \- js目录下准备了两个文件， 主文件和setup文件
  
  ​     \- setup文件是配置文件，内部的配置选项可以自己通过查看文档或注释的方式了解功能
...
8.axios是一个ajax库：体积小，独立

- 属性：error，complete（执行完毕就调用，不管请求是否发送成功）；catch(),类似于fail；

9.jQuery的ajax的第二种使用形式

- 在$.ajax之外写.then(function () {});执行完ajax之后才会执行.then函数；catch(),类似于fail；
- fail()失败时的处理函数，.fail(function (err) {});always(),类似于complete。